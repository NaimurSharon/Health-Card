...
@section('content')
    <div class="space-y-6 p-4 sm:p-6 md:p-8">
            ...
            <div class="table-header px-4 py-4 sm:px-6 sm:py-4">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
                    <div class="w-full sm:w-auto">
                        <h3 class="text-xl sm:text-2xl font-bold text-white leading-tight">Consultation Details</h3>
                        <p class="text-blue-100 text-sm sm:text-base">Video consultation with
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-1 text-sm text-red-700">
                                    <p>This consultation is marked as completed. Please provide the prescription and medical
                                        notes below to finalize the record.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="content-card rounded-lg p-4 sm:p-6 shadow-md border-t-4 border-t-blue-600">
                    <div
                        class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 space-y-2 sm:space-y-0">
                        <h4 class="text-lg sm:text-xl font-bold text-gray-900 flex items-center">
                            <i class="fas fa-file-prescription text-blue-600 mr-2"></i>
                            Prescription & Medical Notes
                        </h4>
                        @if($consultation->status == 'completed' && !$consultation->prescription)
                            <span
                                class="bg-red-100 text-red-800 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">Required</span>
                        @endif
                    </div>
                        ...
                        <form action="{{ route('doctor.video-consultation.prescription', $consultation->id) }}" method="POST">
                            ...
                            <div class="space-y-6">
                                <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                                    <label class="block text-sm sm:text-base font-semibold text-gray-800 mb-2">
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">Please list all prescribed medications, dosages, and
                                        instructions clearly.</p>
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Clinical Notes &
                                        Observations</label>
                                    <textarea name="doctor_notes" rows="3"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:border-gray-500 focus:ring-1 focus:ring-gray-200 text-sm sm:text-base"
                                        placeholder="Patient history, examination findings, diagnosis notes...">{{ old('doctor_notes', $consultation->doctor_notes) }}</textarea>
                                </div>

                                <div
                                    class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-xl border border-gray-200">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Key Medication
                                            Summary</label>
                                        <input type="text" name="medication"
                                            value="{{ old('medication', $consultation->call_metadata['medication'] ?? '') }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-200 text-sm"
                                ...
                                <div
                                    class="flex flex-col-reverse sm:flex-row items-start sm:items-center justify-between pt-4 space-y-3 sm:space-y-0 space-y-reverse">
                                    <p class="text-xs sm:text-sm text-gray-500 italic">
                                        <i class="fas fa-info-circle mr-1"></i>
                                        This will be saved to the patient's medical record.
                                    </p>
                                    <button type="submit"
                                        class="bg-blue-600 text-white px-6 py-2 sm:px-8 sm:py-3 rounded-lg hover:bg-blue-700 transition-all transform hover:scale-105 font-bold shadow-lg flex items-center justify-center w-full sm:w-auto text-sm sm:text-base">
                    ...
                    @else
                        <div class="text-center py-8 sm:py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200 p-4">
                            <i class="fas fa-lock text-3xl sm:text-4xl mb-3 text-gray-300"></i>
                            <h5 class="text-base sm:text-lg font-medium text-gray-600">Prescription Locked</h5>
                            <p class="text-sm text-gray-500 max-w-md mx-auto mt-2">
                                The prescription form will become available once the consultation call has started or is
                                completed.
                            </p>
                        </div>
                    @endif
                        ...
                        <div>
                            <div class="space-y-3 text-sm">
                                <div class="flex justify-between border-b border-gray-100 pb-2">
                                    ...
                                    <span
                                        class="font-mono text-gray-900 truncate max-w-[50%]">{{ $consultation->call_id }}</span>
                                </div>
                                <div class="flex justify-between border-b border-gray-100 pb-2">
        ...
        ($consultation->status == 'ongoing' ? 'bg-blue-100 text-blue-800' :
            ($consultation->status == 'scheduled' ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800')) }}">
                                ...
                                <div class="flex justify-between border-b border-gray-100 pb-2">
                                    ...
                                    <span
                                        class="font-medium text-right">{{ $consultation->scheduled_for->format('M j, Y g:i A') }}</span>
                                </div>
        ...
        ($consultation->payment_status == 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                ...
                                @if($consultation->payment)
                                    <div class="flex justify-between border-b border-gray-100 pb-2">
                                        ...
                                        <span
                                            class="font-medium capitalize">{{ $consultation->payment->payment_method ?? 'N/A' }}</span>
                                    </div>
                                    <div class="flex justify-between border-b border-gray-100 pb-2">
                                        <span class="text-gray-600">Trx ID</span>
                                        <span
                                            class="font-mono text-gray-900 truncate max-w-[50%]">{{ $consultation->payment->transaction_id ?? 'N/A' }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            <div class="content-card rounded-lg p-4 sm:p-6 shadow-sm lg:col-span-1 h-fit">
                <h4 class="text-xl font-semibold text-gray-900 border-b border-gray-200/60 pb-3 mb-4">Patient Information
                </h4>

                <div class="space-y-4">
                    <div class="flex items-center space-x-4">
                        <div
                            class="w-12 h-12 sm:w-16 sm:h-16 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                            <i class="fas fa-user text-blue-600 text-lg sm:text-xl"></i>
                        </div>
                        <div>
                            <h5 class="font-semibold text-gray-900 text-base sm:text-lg">
                                {{ $consultation->student->user->name }}
                            </h5>
                            <p class="text-xs sm:text-sm text-gray-600">Student ID: {{ $consultation->student->student_id }}
                            </p>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 text-sm border-b border-gray-200 pb-4">
                        <div>
                    ...
                    <div class="pt-4 border-t-0 border-gray-200">
                        <h6 class="font-semibold text-gray-900 mb-2 text-base">Medical Information</h6>
                        <div class="space-y-1 text-sm">
                            <p><span class="text-gray-600">Allergies:</span>
                                <span class="font-medium">{{ $consultation->student->allergies ?? 'None reported' }}</span>
                            </p>
                            <p><span class="text-gray-600">Conditions:</span>
                                <span
                                    class="font-medium">{{ $consultation->student->medical_conditions ?? 'None reported' }}</span>
                            </p>
                        </div>
                    </div>

                    <div class="pt-4 border-t border-gray-200">
                        <h6 class="font-semibold text-gray-900 mb-2 text-base">Reported Symptoms</h6>
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                            <p class="text-sm text-gray-700 whitespace-pre-wrap">{{ $consultation->symptoms }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <style>
        ...
        /* Ensure textareas and inputs are not excessively wide on small screens, though w-full handles this */
        textarea,
        input[type="text"],
        input[type="date"] {
            max-width: 100%;
