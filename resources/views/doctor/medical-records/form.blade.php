...
@section('title', isset($medicalRecord) ? 'Edit Medical Record' : 'Add New Medical Record')
...
<div class="space-y-6">
    ...
    <div class="content-card rounded-lg overflow-hidden">
        <div class="table-header px-6 py-4 flex justify-between items-center">
            <h3 class="text-2xl font-bold">{{ isset($medicalRecord) ? 'Edit Medical Record' : 'Add New Medical Record' }}</h3>
            <button type="submit" form="medical-record-form" 
                ...
                <i class="fas fa-save mr-2"></i>{{ isset($medicalRecord) ? 'Update Record' : 'Create Record' }}
            </button>
        </div>
    </div>
    ...
    <div class="content-card rounded-lg p-6 shadow-sm">
        <form id="medical-record-form" 
              action="{{ isset($medicalRecord) ? route('doctor.medical-records.update', $medicalRecord) : route('doctor.medical-records.store') }}" 
              method="POST">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-4">
                    ...
                    @if(isset($medicalRecord))
                        <!-- Student Display (Read-only for edit) -->
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="record_date" id="record_date" 
                                   value="{{ old('record_date', isset($medicalRecord) ? $medicalRecord->record_date->format('Y-m-d') : date('Y-m-d')) }}" required
                                   class="w-full px-4 py-3 bg-white/80 border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200">
                            @error('record_date')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="follow_up_date" id="follow_up_date" 
                                   value="{{ old('follow_up_date', isset($medicalRecord) && $medicalRecord->follow_up_date ? $medicalRecord->follow_up_date->format('Y-m-d') : '') }}"
                                   class="w-full px-4 py-3 bg-white/80 border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200">
            ...
            <div class="mt-8 pt-6 border-t border-gray-200/60 flex justify-end space-x-4">
                <a href="{{ isset($medicalRecord) ? route('doctor.medical-records.show', $medicalRecord) : route('doctor.medical-records.index') }}" 
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>{{ isset($medicalRecord) ? 'Update Medical Record' : 'Create Medical Record' }}
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set today's date as default for record date on create
        @if(!isset($medicalRecord))
