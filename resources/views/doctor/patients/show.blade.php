    ...
    <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3 space-y-6">
                    ...
                    <div class="content-card rounded-lg p-6 shadow-sm">
                        <h4 class="text-xl font-semibold text-gray-900 border-b border-gray-200/60 pb-3 mb-4">Patient
                            Information</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p
                                        class="text-lg text-gray-900 {{ $student->blood_group ? 'text-red-600 font-semibold' : '' }}">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg text-gray-900">
                                        {{ $student->admission_date ? $student->admission_date->format('M j, Y') : 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                @foreach($medicalRecords as $record)
                                                        <div class="p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                                                            <div class="flex justify-between items-start mb-3">
                                                                <div>
                                                                    <span
                                                                        class="text-sm font-medium text-gray-900">{{ $record->record_date->format('F j, Y') }}</span>
                                                                    <span
                                    ...
                                    ($record->record_type == 'vaccination' ? 'bg-green-100 text-green-800' :
                                        'bg-blue-100 text-blue-800') }}">
                                                                ...
                                                                <span class="text-xs text-gray-500">By: {{ $record->recordedBy->name ?? 'Unknown' }}</span>
                                                            </div>

                                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                                                                <div>
                                                                    <p class="text-xs font-medium text-gray-600">Symptoms</p>
                                                                    <p class="text-sm text-gray-700 line-clamp-2">{{ $record->symptoms ?? 'No symptoms recorded' }}</p>
                                                                </div>
                                                                <div>
                                                                    <p class="text-xs font-medium text-gray-600">Diagnosis</p>
                                                                    <p class="text-sm text-gray-700 line-clamp-2">{{ $record->diagnosis ?? 'No diagnosis recorded' }}</p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <!-- Vital Signs -->
                                                            @if($record->height || $record->weight || $record->temperature || $record->blood_pressure)
                                                                <div class="mt-3 pt-3 border-t border-gray-200">
                                                                    <p class="text-xs font-medium text-gray-600 mb-2">Vital Signs</p>
                    ...
                    <div class="content-card rounded-lg p-6 shadow-sm border-l-4 border-green-500">
                        <h4 class="text-xl font-semibold text-gray-900 border-b border-gray-200/60 pb-3 mb-4">Create New Medical
                            Record</h4>

                        <form action="{{ route('doctor.patients.create-medical-record', $student) }}" method="POST"
                            class="space-y-4">
                            @csrf

                            <input type="hidden" name="user_id" value="{{ $student->user->id }}">
                            <input type="hidden" name="patient_type" value="student">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="prescription"
                                        class="block text-sm font-medium text-gray-700 mb-2">Prescription</label>
                                ...
                                <div>
                                    <label for="medication"
                                        class="block text-sm font-medium text-gray-700 mb-2">Medication</label>
                ...
                <div class="space-y-6">
                    <!-- Statistics Card -->
                    <div class="content-card rounded-lg p-6 shadow-sm">
                        <h4 class="text-xl font-semibold text-gray-900 border-b border-gray-200/60 pb-3 mb-4">Patient Statistics
                        </h4>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                                <div>
                                    <p class="text-sm font-medium text-purple-600">Upcoming Consultations</p>
                                    <p class="text-2xl font-bold text-purple-700">{{ $stats['upcoming_consultations'] }}</p>
                                </div>
                    ...
                    @if($upcomingConsultations && $upcomingConsultations->count() > 0)
                        <div class="content-card rounded-lg p-6 shadow-sm">
                            <div class="flex justify-between items-center mb-4">
                                <h4 class="text-xl font-semibold text-gray-900">Upcoming Video Consultations</h4>
                                <span class="text-sm text-gray-600">{{ $upcomingConsultations->count() }}</span>
                            </div>
                                ...
                                @foreach($upcomingConsultations as $consultation)
                                    <div class="p-3 bg-blue-50 rounded-lg">
                                        <div class="flex justify-between items-start mb-1">
                                            <span class="text-sm font-medium text-gray-900">{{ $consultation->getStatusDisplayAttribute() }}</span>
                                            @if($consultation->scheduled_for)
                                                <span class="text-xs text-gray-500">{{ $consultation->scheduled_for->format('g:i A') }}</span>
                                            @endif
                                        </div>
                                        @if($consultation->scheduled_for)
                                            <p class="text-xs text-gray-600">{{ $consultation->scheduled_for->format('M j, Y') }}</p>
                                        @endif
                                        @if($consultation->symptoms)
                                            <p class="text-xs text-gray-700 mt-1 line-clamp-2">{{ $consultation->symptoms }}</p>
                                        @endif
                                        @if($consultation->isAvailable())
                                            <div class="mt-2">
                                                <a href="{{ route('doctor.video-consultations.join', $consultation) }}"
                    ...
                    @if($student->healthCard)
                        ...
                        ($student->healthCard->status == 'expired' ? 'bg-red-100 text-red-800' :
                            'bg-yellow-100 text-yellow-800') }}">
                                            ...
                                            <div class="flex justify-between items-center">
                                                ...
                                                <span
                                                    class="text-sm font-medium {{ $student->healthCard->expiry_date && $student->healthCard->expiry_date->isPast() ? 'text-red-600' : 'text-gray-900' }}">
                                                    @if($student->healthCard->expiry_date)
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Set default date for record_date to today
                ...
                // Set minimum date for follow_up_date to tomorrow
                const followUpDateInput = document.getElementById('follow_up_date');
                if (followUpDateInput) {
                    const tomorrow = new Date();
                    tomorrow.setDate(tomorrow.getDate() + 1);
                    followUpDateInput.min = tomorrow.toISOString().split('T')[0];
                }
                ...
                function calculateBMI() {
                    if (heightInput && weightInput && heightInput.value && weightInput.value) {
                        ...
                        if (!bmiDisplay) {
                            ...
                            weightInput.parentNode.appendChild(bmiDisplay);
                        }
                        ...
                        bmiDisplay.innerHTML = `<span class="font-medium">BMI:</span> <span class="${color} font-bold">${bmi.toFixed(1)}</span> (${category})`;
                    } else {
                ...
                if (heightInput && weightInput) {
                    ...
                    // Calculate initial BMI if values are pre-filled
