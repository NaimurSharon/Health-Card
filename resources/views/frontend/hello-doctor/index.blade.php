...
@section('content')
    <div class="space-y-6" id="hello-doctor-container">
                                ...
                                @if(Auth::check())
                                    ...
                                    @if(isset($upcomingConsultations) && $upcomingConsultations->count() > 0)
                                        <div class="hidden sm:block">
                                            <div class="backdrop-blur-sm text-white py-2 rounded-lg text-sm font-medium">
                                                <span class="font-bold text-white">You have
                                                    {{ $upcomingConsultations->count() }} Upcoming Appointments</span>
                                            </div>
                                        </div>
                                    @endif
                                @else
        ...
        <div class="content-card rounded-lg p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($doctors as $doctor)
                    <div
                        class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
                        ...
                        <div class="relative">
                            ...
                            <div class="absolute top-4 right-4">
                                @if($doctor->today_availability)
                                    <span
                                        class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium flex items-center">
                            ...
                            <div class="space-y-2">
                                @auth
                                    @if(auth()->user()->role === 'student')
                                        ...
                                        @if($doctor->today_availability)
                                            <button
                                                onclick="initiateInstantCall({{ $doctor->id }}, '{{ $doctor->name }}', {{ $doctor->doctorDetail->consultation_fee ?? 500 }})"
                                                ...
                                                <span
                                                    class="ml-auto bg-white/20 px-2 py-0.5 rounded text-xs">৳{{ $doctor->doctorDetail->consultation_fee ?? 500 }}</span>
                                            </button>
                                        @else
                                            <button disabled
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700">
                            <strong>Note:</strong> All consultations are conducted via video call. You'll be able to join
                            the call from your consultations page at the scheduled time.
                        </p>
                    </div>
                </div>
            </div>

            <form id="appointmentBookingForm" action="{{ route('hello-doctor.store-video-consultation') }}" method="POST"
                class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="scheduled_date" id="form_appointment_date"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div
                            class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg text-sm text-gray-600 flex items-center">
                ...
                <div>
                    ...
                    <textarea name="reason" rows="3" required
                        placeholder="Briefly describe the reason for your video consultation"
                ...
                <div class="flex gap-3">
                    <button type="submit"
                        class="flex-1 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition-colors font-medium text-lg">
        ...
        <div id="emergency-form" class="content-card rounded-lg p-6 shadow-sm hidden">
            ...
            <form id="emergencyForm" action="{{ route('student.hello-doctor.store-treatment-request') }}" method="POST"
                class="space-y-4">
                @csrf
                ...
                <button type="submit"
                    class="w-full bg-red-600 text-white py-3 rounded-lg hover:bg-red-700 transition-colors font-medium text-lg">
