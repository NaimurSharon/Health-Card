...
@section('title', 'Create Health Report - ' . $student->user->name)
...
<div class="space-y-6">
    ...
    <div class="content-card rounded-lg overflow-hidden">
        <div class="table-header px-6 py-4 flex justify-between items-center">
            <h3 class="text-2xl font-bold">Create Health Report - {{ $student->user->name }}</h3>
    ...
    <div class="content-card rounded-lg p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                ...
                <div class="text-lg font-bold text-blue-700">{{ $student->student_id }}</div>
            </div>
...
<script>
    ...
    async function saveHealthReport() {
        ...
        saveButton.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Creating...';
        saveButton.disabled = true;
        
        try {
            const response = await fetch('{{ route("admin.health-reports.store") }}', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                showNotification('Health report created successfully!', 'success');
                // Redirect to the created report after a short delay
                setTimeout(() => {
                    window.location.href = result.redirect_url || '{{ route("admin.health-reports.index") }}';
                }, 1500);
            } else {
                showNotification('Error creating health report: ' + (result.message || 'Please check the form'), 'error');
            }
        } catch (error) {
            showNotification('Error creating health report. Please try again.', 'error');
