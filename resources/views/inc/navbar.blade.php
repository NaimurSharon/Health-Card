    ...
    <div class="flex items-center justify-between px-6 py-3">
        <div class="flex items-center">
            <div class="flex items-center space-x-3">
                <img src="{{ asset('public/storage/' . setting('site_favicon')) }}" alt="School Logo"
                    class="w-8 h-8 rounded">
                <div>
                    <h1 class="text-xl font-bold tiro text-gray-900">
                        {{ Auth::user()->school->name ?? setting('site_title') }}
                    </h1>
        ...
        <nav class="hidden lg:flex items-center tiro space-x-1">
            <a href="{{ route('home') }}" class="nav-link {{ Request::is('student/dashboard') ? 'active' : '' }}">
            ...
            @auth
                ...
                @endif
            @endauth
            <a href="{{ route('hello-doctor') }}" class="nav-link {{ Request::is('hello-doctor*') ? 'active' : '' }}">
...
<!-- Health Tip Banner -->
@if($todaystip)
    <div id="healthTipBanner"
        class="bg-yellow-400 text-black px-3 py-2 flex items-center justify-between gap-4 transition-all tiro duration-300">
        <div class="flex items-center gap-4 flex-1">
            <span class="bg-red-600 text-white px-2 py-2 tiro rounded-full text-sm whitespace-nowrap">আজকের স্বাস্থ্য
                পরামর্শ</span>
        ...
        <button onclick="dismissHealthTip()"
            class="text-gray-700 hover:text-gray-900 transition-colors duration-200 flex-shrink-0">
...
<div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
    ...
    <div class="p-4 space-y-2 tiro">
        <a href="{{ route('home') }}" class="nav-link block {{ Request::is('student/dashboard') ? 'active' : '' }}"
        ...
        @auth
            ...
            @endif
        @endauth
        <a href="{{ route('hello-doctor') }}" class="nav-link block {{ Request::is('hello-doctor*') ? 'active' : '' }}"
            @click="mobileMenuOpen = false">
        ...
        <!--</a>-->
        <a href="{{ route('scholarship.register') }}"
            class="nav-link block {{ Request::is('scholarship*') ? 'active' : '' }}" @click="mobileMenuOpen = false">
