...
<div class="space-y-6">
    ...
    <div class="content-card rounded-lg overflow-hidden">
        <div class="table-header px-4 py-4 sm:px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h3 class="text-xl sm:text-2xl font-bold">Edit Notice</h3>
    ...
    <div class="content-card rounded-lg p-4 sm:p-6">
        <form action="{{ route('principal.notices.update', $notice->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            <option value="draft" {{ old('status', $notice->status) == 'draft' ? 'selected' : '' }}>Save as Draft</option>
                            <option value="published" {{ old('status', $notice->status) == 'published' ? 'selected' : '' }}>Publish Now</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="priority" name="priority" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            <option value="low" {{ old('priority', $notice->priority) == 'low' ? 'selected' : '' }}>Low Priority</option>
                            <option value="medium" {{ old('priority', $notice->priority) == 'medium' ? 'selected' : '' }}>Medium Priority</option>
                            <option value="high" {{ old('priority', $notice->priority) == 'high' ? 'selected' : '' }}>High Priority</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            @php
                                $targetRoles = is_array(old('target_roles', $notice->target_roles)) 
                                    ? old('target_roles', $notice->target_roles) 
                                    : [];
                            @endphp
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" id="role_staff" name="target_roles[]" value="staff"
                                       class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                       {{ in_array('staff', $targetRoles) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (expiryDateInput.value < minDate) {
